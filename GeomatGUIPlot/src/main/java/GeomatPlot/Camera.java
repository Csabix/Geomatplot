...
public class Camera {
    ...
    public Camera(GL4 gl, float w, float h, float zoom, float x, float y) {
        ...
        gl.glBindBufferRange(GL_UNIFORM_BUFFER, 0, ubo, 0, (long) data.length * Float.BYTES);
    }
    public void sync(GL4 gl) {
        if(needSync) {
            ByteBuffer byteBuffer = gl.glMapNamedBufferRange(ubo, 0, (long) data.length * Float.BYTES, GL_MAP_WRITE_BIT);
    ...
    public void clean(GL4 gl){
        GLObject.deleteBuffers(gl, new int[]{ubo});
    }
}
