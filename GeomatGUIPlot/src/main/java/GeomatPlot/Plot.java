...
public class Plot extends AbstractWindow{
    ...
    protected void processEvent(GL4 gl,AWTEvent event) {
        switch (event.getID()) {
            ...
            case MOUSE_PRESSED:
                ...
                Optional<Drawable> result = drawerContainer.getClicked(typeID);

                if(result.isPresent() && result.get().isMovable()) {
                    movement = new Movement(result.get(), clickLocation.first, clickLocation.second);
                } else {
                    camera.setOldData();
                }
                break;
    ...
    public void addDrawables(Object[][] drawables) {
        /*Drawable[] flattened = new Drawable[drawables.length];
        for (int i = 0; i < drawables.length; i++) {
            flattened[i] = (Drawable)drawables[i][0];
        }
        addDrawable(flattened);*/
        Drawable[] flattened = new Drawable[drawables[0].length];
        for (int i = 0; i < drawables[0].length; i++) {
            flattened[i] = (Drawable)drawables[0][i];
        }
        addDrawable(flattened);
    }
    public void updateDrawables(Object[][] drawables) {
        /*Drawable[] flattened = new Drawable[drawables.length];
        for (int i = 0; i < drawables.length; i++) {
            flattened[i] = (Drawable)drawables[i][0];
        }
        updateDrawable(flattened);*/
        Drawable[] flattened = new Drawable[drawables[0].length];
        for (int i = 0; i < drawables[0].length; i++) {
            flattened[i] = (Drawable)drawables[0][i];
        }
    ...
    public void removeDrawables(Object[][] drawables) {
        /*Drawable[] flattened = new Drawable[drawables.length];
        for (int i = 0; i < drawables.length; i++) {
            flattened[i] = (Drawable)drawables[i][0];
        }
        removeDrawable(flattened);*/
        Drawable[] flattened = new Drawable[drawables[0].length];
        for (int i = 0; i < drawables[0].length; i++) {
            flattened[i] = (Drawable)drawables[0][i];
        }
    ...
    private void resize(GL4 gl,int sWidth, int sHeight) {
        ...
        ((FunctionDrawer)drawerContainer.getDrawer(DrawableType.Function)).createFBOs(gl);
    }
