...
public class LineDrawer<T extends  gLine> extends Drawer{
    private static final Integer INITIAL_CAPACITY = 300 * gLine.VERTEX_BYTE;
    ...
    protected void deleteInner(GL4 gl, int[] IDs) {
        List<T> keep = new ArrayList<>(syncedDrawables.size() - IDs.length);
        // Update line buffer
        ...
        for (Drawable drawable:syncedDrawables) {
            if(currentID < IDs.length && drawable.getID() == IDs[currentID]) {
                offsets[currentID] = currentOffset;
                lineRanges[currentID] = drawable.bytes();
                ++currentID;
            } else {
                keep.add((T)drawable);
            }
        ...
        for (T line:keep) {
            int indSize = (line.x.length - 1) * 6;
            commands.add(new DrawArraysIndirectCommand(indSize,nextIndex));
            nextIndex += indSize + 18;
        }
    ...
    public void clean(GL4 gl) {
        lineBuffer.clean(gl);
        indirectDrawArraysCommandBuffer.clean(gl);
        shader.delete(gl);
