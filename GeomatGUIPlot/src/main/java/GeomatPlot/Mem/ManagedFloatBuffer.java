...
public class ManagedFloatBuffer extends BaseManagedBuffer{
    ...
    public void update(GL4 gl, List<PackableFloat> elements, Integer first, Integer last) { // [first, last[
        ...
        offset = 0;
        for (PackableFloat packableFloat:updateRange) {
            if(packableFloat.bytes() == packableFloat.lastSize) {
                floatBuffer.position(offset / Integer.BYTES);
                floatBuffer.put(packableFloat.pack());
            }
            offset += packableFloat.lastSize;
        }
