...
v0 = Eval((q0-p0)/Scalar(p,p0,q0,@pPointDistanceCallback)); % Operators: point-point=vector, vector/scalar = vector
Ray(p0,v0,'r',1.5)

pp = p0; n=10; % Sphere tracing illustration:
for i = 1:n
    d = Scalar(p,pp,f,@pPolyDistanceCallback); % distance to polygon yields a dependent scalar value
    pCircle(pp,d,p,'Color',[i/n 1-i/n 0])
    pp = (pp + v0*d)';   % vector*scalar=vector, point+vector=point, ' same as Eval
end

xlim([-1.6 1.3]); ylim([0 1.8]);
