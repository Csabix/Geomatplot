...
public class EventAggregator {
    ...
    public EventAggregator() {
        singleEventList = new LinkedList<>();
    ...
    private void mouseWheelEVT(AWTEvent e) {
        MouseWheelEvent mEvent = null;
        for (AWTEvent event:singleEventList) {
            if(event.getID() == MOUSE_WHEEL) {
        ...
        if(mEvent == null) {
            ...
            MouseWheelEvent newEvent = new MouseWheelEvent((Component)mEvent.getSource(),mEvent.getID(),
                    0,0,0,0,0,0,0,
                    false,mEvent.getScrollType(),mEvent.getScrollAmount(),
                    mEvent.getWheelRotation()+((MouseWheelEvent)e).getWheelRotation(),
                    mEvent.getPreciseWheelRotation() + ((MouseWheelEvent)e).getPreciseWheelRotation());

            singleEventList.remove(mEvent);
            singleEventList.add(newEvent);
        }
    }
}
