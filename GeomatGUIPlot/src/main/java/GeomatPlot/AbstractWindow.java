...
public abstract class AbstractWindow {
    protected static int top,left,bot,right; // Size of the outer frame that the operating system puts around the windows
    public Frame frame;
    ...
    public  AbstractWindow(String title, Integer width, Integer height, Boolean maximized){
        ...
        Thread thread = new Thread(AbstractWindow.this::mainLoop);
        thread.start();
    }
    ...
    protected abstract void close(GL4 gl);
    public void callEvent(AWTEvent event) {
        EventQueue.invokeLater(() -> eventAggregator.addEvent(event));
    }
    public void toFront() {
        frame.toFront();
        frame.requestFocus();
